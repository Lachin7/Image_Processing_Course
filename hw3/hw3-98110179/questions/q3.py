...
## a weighted one :
    ...
    #         for x in range(5):
    #             for y in range(5):
    #                 gaussianKernel[x, y] = np.exp(- (np.power(x, 2) + np.power(y, 2)) / (2 * 1 * 1)) / (
    #                         2 * np.pi * 1 * 1)
    ...
    #         image[mask_y0 + j, mask_x0 + i, :] = np.average(arr, weights=gaussianKernel, axis=2)
...
inpaint(birds, mask_y0= 750, mask_y1=950, mask_x0=800, mask_x1=960, ref_x0=511, ref_x1=781, ref_y0=743, ref_y1=890)


cv2.imwrite('../results/res15.jpg', img)
